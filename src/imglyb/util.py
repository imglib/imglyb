...
def _java_setup():
    ...
    numpy_dtype_to_conversion_method = {
        ...
        np.dtype("bool_"): NumpyToImgLibConversions.toNativeBool,
    ...
    numpy_dtype_to_conversion_with_stride_method = {
        ...
        np.dtype("bool_"): NumpyToImgLibConversionsWithStride.toNativeBool,
...
def to_imglib(source):
    scyjava.start_jvm()
    return ReferenceGuardingRandomAccessibleInterval(
        _to_imglib(source), ReferenceGuard(source)
    )
...
def _to_imglib(source):
    ...
    if source.dtype not in numpy_dtype_to_conversion_method:
        ...
        return numpy_dtype_to_conversion_method[source.dtype](
            long_address, *long_arr_source
        )
    else:
...
def to_imglib_argb(source):
    scyjava.start_jvm()
    return ReferenceGuardingRandomAccessibleInterval(
        _to_imglib_argb(source), ReferenceGuard(source)
    )
