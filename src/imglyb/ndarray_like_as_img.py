...
def _chunk_index_to_slices(shape, chunk_shape, cell_index):
    ...
    slices = tuple(slice(c * cs, (c + 1) * cs) for c, cs in zip(chunk_min, chunk_shape))
...
def _get_chunk_access_array(
    array, chunk_shape, index, chunk_as_array, use_volatile_access=True
):
    try:
        ...
        return accesses.Accesses.asArrayAccess(
            util._get_address(chunk), chunk.size * ratio, use_volatile_access, ptype
        )
...
def as_cell_img(
    ...
    **kwargs,
    ...
    return access_type_function_mapping[access_type](
        array, chunk_shape, chunk_as_array, cache, **kwargs
    )


# TODO is it bad style to use **kwargs to ignore unexpected kwargs?
def as_cell_img_with_array_accesses(
    array, chunk_shape, chunk_as_array, cache, *, use_volatile_access=True, **kwargs
):
    scyjava.start_jvm()

    access_generator = MakeAccessFunction(
        lambda index: _get_chunk_access_array(
            array,
            chunk_shape,
            index,
            chunk_as_array,
            use_volatile_access=use_volatile_access,
        )
    )
...
# TODO is it bad style to use **kwargs to ignore unexpected kwargs?
def as_cell_img_with_native_accesses(
    array, chunk_shape, chunk_as_array, cache, **kwargs
):
    scyjava.start_jvm()
